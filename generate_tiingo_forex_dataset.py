#!/usr/bin/env python3
"""
Generate historical forex dataset from Tiingo API
- Minutely resolution
...
class TiingoForexDataGenerator:
    ...
    def fetch_forex_data(self, ticker: str, start_date: datetime, end_date: datetime) -> List[Dict]:
        """Fetch forex data from Tiingo API for given date range"""
        
        start_formatted = start_date.strftime('%Y-%m-%d')
        end_formatted = end_date.strftime('%Y-%m-%d')
        
        url = f"https://api.tiingo.com/tiingo/fx/prices?tickers={ticker.lower()}&startDate={start_formatted}&endDate={end_formatted}&resampleFreq=1min&token={self.api_key}"
    ...
    def process_raw_data(self, raw_data: List[Dict], trade_pair: TradePair) -> pd.DataFrame:
        """Process raw Tiingo data into a clean DataFrame with market hours filtering and backfilling"""
        ...
        # Fill forward
        result_df['ticker'] = result_df['ticker'].ffill().fillna(trade_pair.trade_pair_id)
    ...
    def generate_dataset(self, months_back: int = 18, output_file: str = 'tiingo_forex_dataset.csv'):
        ...
        print(f"Generating forex dataset from {start_date.date()} to {end_date.date()}")
        ...
        print(f"Estimated API calls: {estimated_chunks} (30-day chunks, conservative approach)")
        ...
        for ticker, trade_pair in self.trade_pairs.items():
            ...
            while current_date < end_date:
                ...
                # Fetch raw data
                raw_data = self.fetch_forex_data(ticker, current_date, chunk_end)
                ...
                if raw_data:
                    ...
                    if not processed_df.empty:
                        ...
                        print(f"    Added {len(processed_df)} data points ({processed_df['is_backfilled'].sum()} backfilled)")
...
def main():
    parser = argparse.ArgumentParser(description='Generate Tiingo forex dataset')
    parser.add_argument('--months', type=int, default=18, help='Number of months back to fetch (default: 18)')
    parser.add_argument('--output', type=str, default='tiingo_forex_dataset.csv', help='Output CSV filename')
    ...
    # Create generator
    generator = TiingoForexDataGenerator(api_key)
    ...
    if args.test:
        ...
        raw_data = generator.fetch_forex_data('EURUSD', start_date, end_date)
