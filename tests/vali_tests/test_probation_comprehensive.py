...
class TestProbationComprehensive(TestBase):
    ...
    def setUp(self):
        ...
        self.CURRENT_TIME = self.START_TIME + ValiConfig.CHALLENGE_PERIOD_MAXIMUM_MS + 1000
    ...
    def test_probation_timeout_elimination(self):
        ...
        self.challengeperiod_manager.active_miners[expired_miner] = (
            ...
            self.PROBATION_EXPIRED - ValiConfig.PROBATION_MAXIMUM_MS - 1000  # 1 second past expiry
        )
    ...
    def test_maincomp_to_probation_to_elimination_flow(self):
        ...
        for checkpoint in poor_ledger[TP_ID_PORTFOLIO].cps:
            ...
            checkpoint.mdd = 0.8     # 20% drawdown (should cause elimination)
        
        self.ledger_manager.save_perf_ledgers({poor_miner: poor_ledger})
        ...
        # Poor miner should either be eliminated (due to drawdown) or demoted to probation
        if poor_miner in eliminated_hotkeys:
            # Eliminated due to drawdown - this is correct behavior
            self.assertNotIn(poor_miner, maincomp_miners)
            self.assertNotIn(poor_miner, probation_miners)
        elif poor_miner in probation_miners:
            # Demoted to probation - test second phase
            self.assertNotIn(poor_miner, maincomp_miners)
            ...
            self.fail(f"Poor miner {poor_miner} should be either eliminated or in probation")
    ...
    def test_probation_elimination_reason_tracking(self):
        ...
        expired_start_time = self.CURRENT_TIME - ValiConfig.PROBATION_MAXIMUM_MS - 5000
    ...
    def test_massive_demotion_scenario_stress_test(self):
        ...
        """
        # Setup scenario where 15 out of 30 maincomp miners perform poorly
        poorly_performing_miners = self.SUCCESS_MINER_NAMES[:15]
        ...
        self.ledger_manager.save_perf_ledgers(poor_ledgers)
        ...
        eliminated_count = len(self.challengeperiod_manager.elimination_manager.get_eliminated_hotkeys())
        total_initial = initial_maincomp + initial_probation + len(self.challengeperiod_manager.get_testing_miners())
        total_final = final_maincomp + final_probation + len(self.challengeperiod_manager.get_testing_miners())
    ...
    def test_probation_to_challenge_transition_prevention(self):
        ...
        probation_miner = "probation_stay_test"
        ...
        for i in range(3):
            ...
            probation_miners = self.challengeperiod_manager.get_probation_miners()
            maincomp_miners = self.challengeperiod_manager.get_success_miners()
            eliminated_miners = self.challengeperiod_manager.elimination_manager.get_eliminated_hotkeys()
