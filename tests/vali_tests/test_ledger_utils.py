...
class TestLedgerUtils(TestBase):
    ...
    def test_daily_return_log(self):
        ...
        cp3 = copy.deepcopy(checkpoints[2])
        ...
        # Test with explicitly constructed checkpoints to test UTC day boundary logic
        num_checkpoints_per_day = int(ValiConfig.DAILY_CHECKPOINTS)
        ...
        day1_start_time = int(day1_date.timestamp() * 1000) + (6 * 3600 * 1000)  # 6:00 AM on day1
        boundary_cp = PerfCheckpoint(
            last_update_ms=day1_start_time + checkpoint_duration,  # 6:00 PM on day1
            accum_ms=checkpoint_duration,  # Exactly the expected duration
            open_ms=day1_start_time,  # Starts at 6:00 AM
            gain=0.15,
            loss=-0.08,
            prev_portfolio_ret=1.0,
            n_updates=1,
        ...
        # One checkpoint won't have a return since we need a full day's worth
        ...
        # Test exact checkpoint count requirements
        ...
        extra_date_returns = LedgerUtils.daily_return_log_by_date(extra_ledger)
