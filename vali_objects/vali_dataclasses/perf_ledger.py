...
class PerfLedgerManager(CacheController):
    ...
    def load_perf_ledgers_from_disk(self, read_as_pydantic=True) -> dict[str, PerfLedger]:
    ...
    def _can_shortcut(self, tp_to_historical_positions: dict[str: Position], end_time_ms: int,
                      realtime_position_to_pop: Position | None, start_time_ms: int, perf_ledger: PerfLedger) -> (bool, float, float, float, int, int):
    ...
    def update_all_perf_ledgers(self, hotkey_to_positions: dict[str, List[Position]],
        ...
        if return_dict:
            ...
            self.write_perf_ledgers_to_disk(existing_perf_ledgers)
    ...
    def load_perf_ledgers_from_memory(self):
        if self.hotkey_to_perf_ledger is None:
            self.hotkey_to_perf_ledger = self.load_perf_ledgers_from_disk(read_as_pydantic=False)
    ...
    def write_perf_ledgers_to_disk(self, perf_ledgers: dict[str, PerfLedger] | dict[str, dict]):
