...
class ChallengePeriodManagerServer(CacheController):
    """
    Server-side implementation of ChallengePeriodManager.

    Uses local dicts instead of IPC managerized dicts for 50-200x better performance.
    All dict operations happen server-side - clients access via RPC methods.
    """
    ...
    def health_check_rpc(self) -> dict:
        ...
        Returns:
            dict: Health status with timestamp and metrics
        """
        return {
            "status": "ok",
            "timestamp_ms": TimeUtil.now_in_millis(),
            "active_miners_count": len(self.active_miners),
            "elimination_reasons_count": len(self.eliminations_with_reasons)
        }
    ...
    def get_miner_bucket_rpc(self, hotkey: str) -> Optional[str]:
        ...
        Returns:
            str: Bucket value (enum value), or None if not found
        """
    ...
    def get_miner_previous_bucket_rpc(self, hotkey: str) -> Optional[str]:
        ...
        Returns:
            str: Previous bucket enum value, or None if not found or not set
        """
        info = self.active_miners.get(hotkey)
        if info and info[2]:
            return info[2].value
    ...
    def get_hotkeys_by_bucket_rpc(self, bucket_value: str) -> List[str]:
        ...
        Args:
            bucket_value: Bucket enum value (e.g., "CHALLENGE", "MAINCOMP")
    ...
    def set_miner_bucket_rpc(
        ...
        Args:
            ...
            bucket_value: The current bucket (enum value)
            start_time: Bucket start time in milliseconds
            prev_bucket_value: Previous bucket (enum value, for plagiarism demotions)
    ...
    def update_miners_rpc(self, miners_dict: dict) -> int:
        ...
        Args:
            miners_dict: Dict mapping hotkeys to dicts with keys:
                - bucket: str (enum value)
                - start_time: int
                - prev_bucket: Optional[str] (enum value)
        ...
        """
        count = 0
        for hotkey, info in miners_dict.items():
            bucket = MinerBucket(info["bucket"])
            start_time = info["start_time"]
            prev_bucket = MinerBucket(info["prev_bucket"]) if info.get("prev_bucket") else None
            prev_time = info.get("prev_time")
    ...
    def sync_challenge_period_data_rpc(self, active_miners_sync: dict) -> None:
        ...
        """
        self.sync_challenge_period_data(active_miners_sync)
    ...
    def meets_time_criteria_rpc(self, current_time: int, bucket_start_time: int, bucket_value: str) -> bool:
        ...
        Args:
            ...
            bucket_value: Bucket enum value (e.g., "CHALLENGE", "PROBATION")

        Returns:
            bool: True if miner meets time criteria
        """
        bucket = MinerBucket(bucket_value)
        return self.meets_time_criteria(current_time, bucket_start_time, bucket)
    ...
    def update_plagiarism_miners_rpc(self, current_time: int, plagiarism_miners: dict) -> None:
        ...
        """
        self.update_plagiarism_miners(current_time, plagiarism_miners)
    ...
    def prepare_plagiarism_elimination_miners_rpc(self, current_time: int) -> dict:
        ...
        Returns:
            dict: Mapping hotkeys to (reason, drawdown) tuples
        """
        return self.prepare_plagiarism_elimination_miners(current_time)

    def _demote_plagiarism_in_memory_rpc(self, hotkeys: list, current_time: int) -> None:
        """
        ...
        Args:
            ...
            current_time: Current time in milliseconds
        """
        self._demote_plagiarism_in_memory(hotkeys, current_time)

    def _promote_plagiarism_to_previous_bucket_in_memory_rpc(self, hotkeys: list, current_time: int) -> None:
        """
        ...
        Args:
            ...
            current_time: Current time in milliseconds
        """
        self._promote_plagiarism_to_previous_bucket_in_memory(hotkeys, current_time)

    def _eliminate_challengeperiod_in_memory_rpc(self, eliminations_with_reasons: dict) -> None:
        """
        ...
        """
        self._eliminate_challengeperiod_in_memory(eliminations_with_reasons)

    def _add_challengeperiod_testing_in_memory_and_disk_rpc(
        self,
        new_hotkeys: list,
        eliminations: list,
    ...
    def _promote_challengeperiod_in_memory_rpc(self, hotkeys: list, current_time: int) -> None:
        """
        ...
        Args:
            ...
            current_time: Current time in milliseconds
        """
        self._promote_challengeperiod_in_memory(hotkeys, current_time)
    ...
    def inspect(
        ...
        for hotkey, bucket_start_time in inspection_hotkeys.items():
            if ChallengePeriodManagerServer.is_recently_re_registered(portfolio_only_ledgers.get(hotkey), hotkey, hk_to_first_order_time):
            ...
            if miner_bucket is None:
                bt.logging.warning(f'Hotkey {hotkey} has no miner bucket. Skipping inspection.')
                continue
            ...
            has_minimum_ledger, inspection_ledger = ChallengePeriodManagerServer.screen_minimum_ledger(portfolio_only_ledgers, hotkey)
            ...
            has_minimum_positions, inspection_positions = ChallengePeriodManagerServer.screen_minimum_positions(positions, hotkey)
    ...
    # ============================================================================
    # Internal getter/setter methods (used by server methods)
    # ============================================================================
    ...
    def set_miner_bucket(
        ...
        """
        Set or update a miner's bucket information (internal method).
...
def start_challengeperiod_manager_server(
    metagraph,
    perf_ledger_manager,
    position_manager,
    contract_manager,
    ...
    server_instance = ChallengePeriodManagerServer(
        metagraph=metagraph,
    ...
    class ChallengePeriodManagerRPC(BaseManager):
        pass

    # Register the server instance
    ChallengePeriodManagerRPC.register(
        'ChallengePeriodManagerServer',
        callable=lambda: server_instance
    )

    # Create and start RPC server
    manager = ChallengePeriodManagerRPC(address=address, authkey=authkey)
