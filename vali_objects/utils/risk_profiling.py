...
class RiskProfiling:
    ...
    def monotonic_positions(position: Position) -> Position:
        ...
        result_position = copy.deepcopy(position)
        position_orders = [order for order in position.orders]
        ...
        is_long = position.position_type == OrderType.LONG
        ...
        for i in range(1, final_active_order):
            ...
            is_losing = order_price_direction * position_direction < 0
            ...
            if is_losing and leverage_increased_beyond_max:
                order_copy = copy.deepcopy(current_order)
                # Store the aggregate leverage for monotonicity check later
                order_copy.leverage = new_aggregate_leverage  # Store aggregate leverage
