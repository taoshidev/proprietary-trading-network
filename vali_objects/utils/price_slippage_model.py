...
class PriceSlippageModel:
    ...
    def get_bars_with_features(self, trade_pair: TradePair, processed_ms: int, adv_lookback_window: int=10, calc_vol_window: int=30, trading_days_in_a_year: int=252) -> pd.DataFrame:
        ...
        """
        order_date = self.ms_to_date_string(processed_ms)
        start_date = self.get_starting_trading_date(order_date, max(ValiConfig.AVERAGE_DAILY_VOLUME_LOOKBACK_DAYS, ValiConfig.ANNUALIZED_VOLATILITY_LOOKBACK_DAYS) + 1)
