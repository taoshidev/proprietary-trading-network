...
class Position(BaseModel):
    ...
    @classmethod
    def from_dict_without_validation(cls, data: dict):
        # Create an instance without running validation
        instance = cls.__new__(cls)
        for key, value in data.items():
            setattr(instance, key, value)
        # Set the necessary pydantic attributes manually
        instance.__pydantic_initialised__ = True
        instance.__pydantic_fields_set__ = set(data.keys())
        return instance

    @field_validator('net_leverage', mode="before")
    @classmethod
    def validate_leverage(cls, v, values):
        ...
        if open_ms and open_ms < 1720464933743:
            cls._check_and_add_extra_order(values)

        trade_pair = values.get('trade_pair')
        if trade_pair and not (trade_pair.min_leverage <= abs(v) <= trade_pair.max_leverage):
            raise ValueError(f"Net leverage must be between {trade_pair.min_leverage} and {trade_pair.max_leverage}, provided - [{v}]")
        return v
    
    @classmethod
    def _check_and_add_extra_order(cls, values):
