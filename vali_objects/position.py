...
CRYPTO_CARRY_FEE_PER_INTERVAL = (8.0 / (365.0 * 3))  # 8% per year for 1x leverage. Each interval is 8 hrs
FOREX_CARRY_FEE_PER_INTERVAL = 1.75 / 365.0  # 1.75% per year for 1x leverage. Each interval is 24 hrs
INDICES_CARRY_FEE_PER_INTERVAL = 3.0 / 365.0  # 3% per year for 1x leverage. Each interval is 24 hrs
    ...
    def forex_indices_carry_fee(self, current_time_ms: int) -> tuple[float, int]:
        ...
        for n in range(n_intervals_elapsed):
            ...
            if day_of_week_index in (5, 6):
                ...
                fee = self.max_leverage_seen_in_interval(start_ms, end_ms)
                if self.trade_pair.is_forex:
                    fee *= FOREX_CARRY_FEE_PER_INTERVAL
                elif self.trade_pair.is_indices:
                    fee *= INDICES_CARRY_FEE_PER_INTERVAL
                else:
                    raise ValueError(f"Unexpected trade pair: {self.trade_pair.trade_pair_id}")
                if day_of_week_index == 2:
                    fee *= 3.0 # triple fee on Wednesday
