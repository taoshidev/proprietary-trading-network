...
def generate_miner_statistics_data(time_now: int = None):
    if time_now is None:
    ...
    lookback_positions = PositionUtils.filter(
        filtered_positions,
        evaluation_time_ms=time_now
    )

    lookback_positions_recent = PositionUtils.filter_recent(
        filtered_positions,
        evaluation_time_ms=time_now
    )
    ...
    for hotkey, hotkey_ledger in filtered_ledger.items():
        ...
        short_adjusted_return_dict[hotkey] = Scoring.risk_adjusted_return(
        ...
        positional_realized_returns_ratios[hotkey] = PositionUtils.returns_ratio(miner_positions)
        positional_realized_returns_penalties[hotkey] = PositionUtils.returns_ratio_penalty(miner_positions)

        positional_return_time_consistency_ratios[hotkey] = PositionUtils.time_consistency_ratio(miner_positions)
        positional_return_time_consistency_penalty = PositionUtils.time_consistency_penalty(miner_positions)
    ...
    filtered_positions = subtensor_weight_setter.filtered_positions(hotkeys=challengeperiod_success_hotkeys)
    ...
    short_adjusted_return_rank = rank_dictionary(short_adjusted_return_dict)
    short_adjusted_return_percentile = percentile_rank_dictionary(short_adjusted_return_dict)
    ...
    for miner_id in all_miner_hotkeys:
        ...
        miner_data = {
            ...
            "scores": {
                ...
                "short_adjusted_return": {
                    "value": short_adjusted_return_dict.get(miner_id),
                    "rank": short_adjusted_return_rank.get(miner_id),
                    "percentile": short_adjusted_return_percentile.get(miner_id),
                },
    ...
    final_dict = {
        ...
        # "constants": json.dumps(ldconfig_data),
...
def generate_request_minerstatistics(time_now: int):
    final_dict = generate_miner_statistics_data(time_now)
